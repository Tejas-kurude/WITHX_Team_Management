...
function statusClass(status?:string){ 
  ...
  if(s==='PRESENT') return 'bg-green-100/70 text-green-700 border border-green-200/70'; 
  if(s==='LATE') return 'bg-amber-100/70 text-amber-700 border border-amber-200/70'; 
  if(s==='LEAVE') return 'bg-red-100/70 text-red-700 border border-red-200/70'; 
  return 'bg-slate-100/70 text-slate-700 border border-slate-200/70'; 
} 
...
export default function Attendance(){ 
  ...
  async function load(q=search){try{setErr('');const [a,b]=await Promise.all([api.get('/attendance/today'),api.get('/attendance',{params:{search:q||undefined,status:statusFilter||undefined,date:dateFilter||undefined}})]);setToday(a.data);setRows(b.data);if(a.data?.attendance_mode)setMode(a.data.attendance_mode)}catch(e){setErr(messageOf(e))}} 
  useEffect(()=>{void load('')},[]); 
